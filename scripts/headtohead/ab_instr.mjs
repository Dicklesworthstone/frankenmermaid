...
function run(bin) {
  ...
  return {
    ...
    outputs: items
      .map((r) => `${r.id}:${r.output_sha256 ?? r.parse_result_sha256 ?? "none"}`)
      .join(" "),
    items: items.length,
  };
}
...
if (disk.base === disk.cand) {
  console.error(
    "REFUSED: baseline and candidate are the same ELF — the sweep would measure the baseline twice",
  );
  process.exit(3);
}
...
function assertRevision(arm, result) {
  ...
  if (!result.buildRevision) {
    console.error(
      `REFUSED: ${arm} reports no build revision, so --expect-${arm} cannot be checked`,
    );
    process.exit(3);
  }
...
for (let round = 0; round < rounds; round += 1) {
  ...
  for (const arm of order) {
    ...
    if (!selfChecked) {
      if (result.selfElf !== disk[arm]) {
        console.error(
          `REFUSED: ${arm} self-reported ${result.selfElf} but the file on disk is ${disk[arm]}`,
        );
        process.exit(3);
      }
      ...
      if (result.instructions < 1e8) {
        console.error(
          `REFUSED: ${arm} retired only ${result.instructions} instructions — that is not this workload`,
        );
        process.exit(3);
      }
      if (result.items === 0) {
  ...
  for (const arm of order) {
    if (seen[arm].outputs !== outputs) {
      console.error(
        `OUTPUT DIVERGENCE on ${arm} round ${round}:\n  ${outputs}\n  ${seen[arm].outputs}`,
      );
      process.exit(4);
    }
  }
  ...
  console.log(
    `round ${round} order=${order.join(",")} base=${seen.base.instructions} cand=${seen.cand.instructions} null=${seen.null.instructions}  A/B=${r}  A/A=${n}`,
  );
}
