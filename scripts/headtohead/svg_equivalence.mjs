...
export function diffMultisets(left, right, limit = 12) {
  ...
  for (const key of [...keys].sort()) {
    ...
    if (l !== r)
      differences.push({
        value: key.length > 120 ? `${key.slice(0, 117)}...` : key,
        left: l,
        right: r,
      });
  }
...
export function isSyntheticNode(id) {
  ...
  // two of them.
  return (
    /^(?:__)?(?:root[_-])?(?:state[_-])?(?:start|end)(?:__.*)?$/.test(id) || /^root[_-]/.test(id)
  );
}
...
function openGroups(svg) {
  ...
  for (let m = re.exec(svg); m !== null; m = re.exec(svg))
    out.push({ tag: m[0], at: m.index, end: re.lastIndex });
  return out;
}
...
function groupSubtree(svg, from, openEnd) {
  ...
  for (let m = re.exec(svg); m !== null; m = re.exec(svg)) {
    if (m[0][1] === "/") {
      depth -= 1;
      if (depth === 0) return svg.slice(from, re.lastIndex);
...
function inheritanceMarkerDefinition(engine, id, slot, definitions) {
  ...
  const effective = slot === "start" && orient === "auto-start-reverse" ? -intrinsic : intrinsic;
...
function classMarkerKind(engine, marker, slot, definitions) {
  ...
  if (engine === "mermaid-js") {
    const emitted =
      /class-(aggregation|composition|extension|dependency)(?:Start|End)(?:-margin)?$/i
        .exec(id)?.[1]
        ?.toLowerCase();
    const kind =
      emitted === "extension" ? "inheritance" : emitted === "dependency" ? "association" : emitted;
    if (kind === "inheritance") {
  ...
  if (/(?:^|-)arrow-diamond-open$/i.test(id)) {
    ...
    return invalid
      ? invalidMarker("aggregation", invalid)
      : { kind: "aggregation", status: "known" };
  }
  if (/(?:^|-)arrow-diamond$/i.test(id)) {
    const invalid = frankenMarkerDefinition("composition", id, definitions);
    return invalid
      ? invalidMarker("composition", invalid)
      : { kind: "composition", status: "known" };
  }
  ...
  if (/(?:^|-)arrow-end$/i.test(id)) {
    ...
    return invalid
      ? invalidMarker("association", invalid)
      : { kind: "association", status: "known" };
  }
...
function classRelationshipSemantics(engine, edges, nodes, definitions) {
  ...
  for (const edge of edges) {
    ...
    const geometric = from !== null && to !== null ? { from: pseudo(from), to: pseudo(to) } : null;
    const declared =
      engine === "mermaid-js"
...
export function overlappingNodeBoxes(anchors) {
  ...
  let unmeasurable = 0;
  for (const [id, anchor] of anchors) {
    if (Array.isArray(anchor) || !anchor.bounds) {
      unmeasurable += 1;
      continue;
    }
...
export function groundTruth(text) {
  ...
  if (/^sequenceDiagram\b/.test(header)) {
    ...
    for (const rawLine of text.split("\n")) {
      ...
      const message = /^([A-Za-z_][\w-]*?)\s*(?:-->>|->>|-->|->)\s*([^\s:]+)\s*:\s*(.+)$/.exec(
        line,
      );
      if (!message) continue;
  ...
  for (const rawLine of text.split("\n")) {
    ...
    if (matched) continue;
    const decl = new RegExp(`^(${NODE})\\s*(?:\\[[^\\]]*\\]|\\([^)]*\\)|\\{[^}]*\\})?\\s*$`).exec(
      line,
    );
    if (decl) nodes.add(decl[1]);
  }
...
export function compareDiagram({ index, family, fmSvg, jsSvg, source }) {
  ...
  const jsComparableStatus =
    js.topology !== null ? js.topology_status : js.declared_topology_status;
  ...
  const tier2Missing =
    requiredTier2Invariants !== undefined &&
    requiredTier2Invariants.some((required) => !decided.some((c) => c.invariant === required));
...
export function verifyFrankenmermaidAgainstSource({ index, family, fmSvg, source }) {
  ...
  const renderedNodeIds =
    clusterStateLabels.length > 0 ? [...fm.node_ids, ...clusterStateLabels] : fm.node_ids;
  ...
  if (family === "class") {
    const relationshipsDecidable =
      truth?.class_relationships !== undefined && fm.class_relationships !== null;
    ...
    checks.push({
      ...
      detail: relationships ?? {
        reason:
          truth?.class_relationships === undefined
            ? "class_input_not_decodable"
  ...
  const missing = required.filter(
    (invariant) => !decided.some((check) => check.invariant === invariant),
  );
...
export function summarizeFrankenmermaidValidation(results) {
  ...
  for (const result of results) {
    const row = families.get(result.family) ?? {
      diagrams: 0,
      verified: 0,
      divergent: 0,
      unverified: 0,
  ...
  return {
    ...
    claim:
      "source-grounded native-output validation for incumbent-DNF rows: decoded node ids, " +
...
export function summarize(results) {
  ...
  return {
    ...
    claim:
      "engine-neutral structural equivalence: rendered-text multiset (all families), " +
    ...
    by_family: Object.fromEntries(families),
    divergent_samples: divergent.slice(0, 5),
    unverified_samples: unverified
      .slice(0, 5)
      .map((r) => ({ index: r.index, family: r.family, reason: r.unverified_reason })),
  };
}
...
function fixturePair() {
  const js =
    `<svg id="d"><style>.node{fill:#fff}</style><g class="root"><g class="nodes">` +
    ...
    `</g></g></svg>`;
  const fm =
    `<svg><style>.fm-node{fill:#fff}</style>` +
...
const failedInvariants = (result) =>
  result.checks
    .filter((c) => c.pass === false && c.decided && c.gating !== false)
    .map((c) => c.invariant);
...
export function selfTest() {
  ...
  record(
    "declared_path_fallback_is_equivalent",
  ...
  record(
    "er_declaration_resolves_through_kind_prefix",
  ...
  record(
    "er_declaration_resolves_to_the_RIGHT_pair",
  ...
  record(
    "er_declaration_naming_an_absent_entity_still_fails",
  ...
  record(
    "plain_declaration_still_resolves_unchanged",
  ...
  record(
    "unknown_plain_declaration_still_fails",
  ...
  record("native_source_validation_baseline_is_verified", nativeBase.verdict === "verified", {
    verdict: nativeBase.verdict,
    checks: nativeBase.checks,
  });
  record(
    "native_source_validation_summary_passes",
  ...
  record(
    "baseline_geometry_matches_declared_ids",
    base.checks.some(
      (c) => c.invariant === "incumbent_geometry_matches_declared_ids" && c.pass === true,
    ),
  ...
  record(
    "class_relationship_baseline_is_equivalent",
    classBase.verdict === "equivalent" &&
      classBase.checks.some(
        (c) => c.invariant === "class_relationship_semantics_cross_engine" && c.pass === true,
      ),
  ...
  record("native_class_source_validation_is_verified", nativeClassBase.verdict === "verified", {
    verdict: nativeClassBase.verdict,
    checks: nativeClassBase.checks,
  });
  ...
  record(
    "class_relationship_kind_mutation_is_divergent",
  ...
  record(
    "native_class_relationship_kind_mutation_is_divergent",
    nativeWrongClassKind.verdict === "divergent" &&
      nativeWrongClassKind.checks.some(
        (check) =>
  ...
  record(
    "class_relationship_owning_end_mutation_is_divergent",
  ...
  record(
    "class_relationship_marker_drop_is_divergent",
  ...
  record(
    "class_inheritance_as_association_is_divergent",
  ...
  record(
    "class_relationship_kind_swap_is_divergent",
  ...
  record(
    "unknown_class_relationship_marker_is_divergent",
  ...
  const inwardInheritance = classPair.fm.replace('orient="auto-start-reverse"', 'orient="auto"');
  ...
  record(
    "inward_inheritance_triangle_is_divergent",
  ...
  record(
    "filled_inheritance_triangle_is_divergent",
  ...
  record(
    "filled_aggregation_diamond_is_divergent",
  ...
  record(
    "hollow_composition_diamond_is_divergent",
  ...
  record(
    "non_diamond_composition_marker_is_divergent",
  ...
  const droppedEdge = fm.replace(
    '<g id="fm-edge-1" class="fm-edge" data-fm-edge-id="1"><path d="M155,20L245,20"/></g>',
    "",
  );
  const m2 = compareDiagram({
    index: 0,
    family: "flowchart",
    fmSvg: droppedEdge,
    jsSvg: js,
  ...
  record(
    "native_source_validation_dropped_edge_is_divergent",
    nativeDroppedEdge.verdict === "divergent" &&
      nativeDroppedEdge.checks.some(
        (check) =>
  ...
  const droppedJsEdge = js.replace(
    '<path d="M55,20L145,20" class="flowchart-link" data-id="L_A_B_0"/>',
    "",
  );
  const m3 = compareDiagram({
    index: 0,
    family: "flowchart",
    fmSvg: fm,
    jsSvg: droppedJsEdge,
  ...
  record(
    "incumbent_dropped_edge_is_divergent",
  ...
  // to the wrong anchor.
  const mislaid = fm.replace(
    '<g id="fm-node-b-1" class="fm-node fm-node-shape-rect" transform="translate(150, 20)">',
  ...
  record("mislaid_node_is_not_certified", m5.verdict !== "equivalent", {
    verdict: m5.verdict,
    failed: failedInvariants(m5),
    reason: m5.unverified_reason,
  });
  ...
  const extra = fm.replace(
    "</svg>",
    '<text x="1" y="1">0..*</text><text x="2" y="2">1</text></svg>',
  );
  ...
  const merged = fm.replace(
    '<text x="0" y="0">Beta</text>',
    '<text x="0" y="0">Beta extra-word</text>',
  );
  const jsSplit = js.replace("<p>Beta</p>", "<p>Beta</p><p>extra-word</p>");
  const n2 = compareDiagram({
    index: 0,
    family: "flowchart",
    fmSvg: merged,
    jsSvg: jsSplit,
  ...
  record("segmentation_difference_is_not_a_failure", n2.verdict === "equivalent", {
    verdict: n2.verdict,
    failed: failedInvariants(n2),
  });

  // CLASS NEGATIVE CONTROL -- DOM path order is not relationship identity.
  const classEdgeGroups = [...classPair.fm.matchAll(/<g class="fm-edge"[\s\S]*?<\/g>/g)].map(
    (match) => match[0],
  );
  ...
  record("class_path_order_is_not_semantic", classN1.verdict === "equivalent", {
    verdict: classN1.verdict,
    failed: failedInvariants(classN1),
  });

  // CLASS NEGATIVE CONTROL -- cardinality and relationship-label syntax preserve kind/endpoints.
  const classWithLabels = classPair.source.replace("C0 *-- C1", 'C0 "1" *-- "0..*" C1 : owns');
  ...
  record(
    "class_cardinality_and_label_preserve_relationship_semantics",
  ...
  record(
    "overlapping_node_boxes_are_detected",
    (() => {
  ...
  record(
    "unboundable_nodes_are_reported_not_assumed_clean",
    (() => {
      const sig = signature(
      ...
      return (
        sig.overlapping_node_boxes_measured === 0 &&
        sig.overlapping_node_boxes_unmeasurable === 2 &&
  ...
  record(
    "entities_decode",
    decodeEntities("a&lt;b&amp;c&#65;&#x42;") === "a<b&cAB",
    decodeEntities("a&lt;b&amp;c&#65;&#x42;"),
  );
  record(
    "style_text_excluded",
    !visibleTextRuns("<svg><style>.a{fill:red}</style><text>X</text></svg>")
      .visible.join("|")
      .includes("fill"),
  ...
  record(
    "nested_html_label_counted_once",
    visibleTextRuns('<svg><span class="nodeLabel"><p>Node 0</p></span></svg>').visible.join("|") ===
      "Node 0",
    visibleTextRuns('<svg><span class="nodeLabel"><p>Node 0</p></span></svg>').visible,
  );
  record(
    "literal_greater_than_is_visible_text",
    visibleTextRuns("<svg><text>Parse &lt;config></text></svg>").visible.join("|") ===
      "Parse <config>",
  ...
  record(
    "relative_command_refuses",
    pathEndpoints("M0,0l10,10") === null,
    pathEndpoints("M0,0l10,10"),
  );
  record(
    "ambiguous_anchor_refuses",
    nearestAnchor(
      [10, 0],
      [
        ["a", [0, 0]],
  ...
  record(
    "near_tie_and_coincident_anchors_are_distinguished",
    (() => {
      const nearTie = resolveAnchor(
        [10, 0],
        [
          ["a", [0, 0]],
          ["b", [20, 0]],
      ...
      return (
        nearTie.reason === "near_tie" &&
        nearTie.id === null &&
        coincident.reason === "coincident_anchors" &&
        coincident.id === null &&
  ...
  record(
    "wide_rectangle_boundary_beats_neighbour_center",
    nearestAnchor(
      [200, 0],
      [
        [
  ...
  record(
    "overlapping_rectangle_boundaries_refuse",
    nearestAnchor(
      [100, 0],
      [
        [
  ...
  record(
    ...
    [
      canonicalNodeId("mermaid-js", "class50_r14_0-classId-C0-2350"),
      canonicalNodeId("frankenmermaid", "fm-node-c0-0"),
    ],
  );
  record(
    "renderer_state_pseudo_ids_are_synthetic",
  ...
  record(
    "authored_data_id_beats_lossy_element_id",
    nodeIdFromGroup(
      "frankenmermaid",
    ...
    [
      nodeIdFromGroup("frankenmermaid", '<g id="fm-node-s0-e0-0" class="fm-node" data-id="S0_E0">'),
      nodeIdFromGroup("mermaid-js", '<g id="diagram-flowchart-A-7-19" class="node" data-id="A-7">'),
    ],
  );
  record(
    "clickable_mermaid_node_uses_its_anchor_wrapper_transform",
    (() => {
      const svg =
        '<svg><a xlink:href="https://example.com/" transform="translate(50, 20)">' +
  ...
  record(
    "ground_truth_reads_chained_edges",
    (() => {
  ...
  record(
    "ground_truth_reads_edge_labels",
    (() => {
      const t = groundTruth("flowchart LR\n  A-->|goes to|B\n");
  ...
  record(
    "ground_truth_reads_class_relationship_semantics",
    (() => {
  ...
  record(
    "ground_truth_reads_state_transitions",
    (() => {
  ...
  record(
    "ground_truth_reads_er_relationships",
    (() => {
  ...
  {
    const aliased = groundTruth(
      "sequenceDiagram\n  participant C as Client\n  participant S as Server\n  C->>S: hi\n",
    );
    const actorAliased = groundTruth(
      "sequenceDiagram\n  actor A as Alice\n  participant B\n  A->>B: hi\n",
    );
    const plain = groundTruth(
      "sequenceDiagram\n  participant Client\n  participant Server\n  Client->>Server: hi\n",
    );
    record(
      "participant_alias_contributes_its_display_name_as_a_token",
      aliased !== null &&
        aliased.sequence_tokens.includes("Client") &&
        aliased.sequence_tokens.includes("Server") &&
        !aliased.sequence_tokens.includes("C") &&
        !aliased.sequence_tokens.includes("S"),
      aliased?.sequence_tokens,
    );
    record(
      "participant_alias_keeps_its_identity_in_node_ids",
      aliased !== null && aliased.node_ids.join(",") === "c,s",
      aliased?.node_ids,
    );
    record(
      "actor_alias_is_read_like_a_participant_alias",
      actorAliased !== null &&
        actorAliased.sequence_tokens.includes("Alice") &&
        actorAliased.node_ids.includes("a") &&
        actorAliased.node_ids.includes("b"),
      actorAliased?.node_ids,
    );
    record(
      "an_unaliased_participant_still_contributes_its_own_name",
      plain !== null &&
        plain.sequence_tokens.includes("Client") &&
        plain.node_ids.join(",") === "client,server",
  ...
  record(
    "composite_scoped_pseudo_states_are_synthetic",
  ...
  record(
    "a_real_state_named_like_a_pseudo_state_is_not_filtered",
  ...
  );
  {
    ...
    const svg =
      ...
      [
        ...
        "Validating",
        "__state_start",
        "__state_start__state_Processing",
        "__state_end__state_Processing",
      ]
        .map(node)
        .join("") +
      '<rect id="fm-cluster-0" class="fm-cluster"/><text class="fm-cluster-label">Processing</text>' +
      "</svg>";
    const verdict = verifyFrankenmermaidAgainstSource({
      index: 0,
      family: "state",
      fmSvg: svg,
      source: compositeSource,
    });
    const nodeCheck = verdict.checks.find(
      (c) => c.invariant === "node_id_set_vs_input__frankenmermaid",
    );
    record(
      "composite_state_container_counts_as_its_state",
      nodeCheck?.decided === true && nodeCheck.pass === true,
      nodeCheck?.detail,
    );
    ...
    const missingVerdict = verifyFrankenmermaidAgainstSource({
      index: 0,
      family: "state",
      fmSvg: missing,
      source: compositeSource,
    ...
    record(
      "composite_state_container_does_not_hide_a_dropped_state",
      missingVerdict.checks.find((c) => c.invariant === "node_id_set_vs_input__frankenmermaid")
        ?.pass === false,
  ...
  // it now decodes 10 / 9 and verifies.
  {
    ...
    for (const [label, source, from, to] of forms) {
      ...
      record(
        `flowchart_truth_decodes_${label.replace(/\s+/g, "_")}`,
        truth !== null &&
          truth.node_ids.includes(from) &&
          truth.node_ids.includes(to) &&
    ...
    record(
      "bidirectional_arrow_keeps_its_source_endpoint",
      bidi !== null && bidi.node_ids.join(",") === "b,j",
      bidi,
    );
  }
  ...
  // honest — a gate that fails everything is not a gate.
  {
    ...
    const whole = verifyFrankenmermaidAgainstSource({
      index: 0,
      family: "er",
      fmSvg: erWhole,
      source: erSource,
    });
    const stripped = verifyFrankenmermaidAgainstSource({
      index: 0,
      family: "er",
      fmSvg: erStripped,
      source: erSource,
    });
    const crossEngine = compareDiagram({
      index: 0,
      family: "er",
      fmSvg: erStripped,
      jsSvg: erWhole,
    ...
    record(
      "er_dropping_every_relationship_is_not_verified",
      stripped.verdict === "unverified" &&
        !stripped.checks.some(
          (check) => check.decided && check.invariant === "edge_topology_vs_input__frankenmermaid",
        ),
      stripped.verdict,
    );
    record(
      "er_relationships_mermaid_draws_and_we_do_not_is_not_equivalent",
      crossEngine.verdict !== "equivalent",
      crossEngine.verdict,
    );
  }
  record(
    "native_untransformed_rect_nodes_anchor",
    (() => {
      const s = signature(
        '<svg><g id="fm-node-s0-1" class="fm-node" data-id="S0"><rect x="10" y="20" width="30" height="40"/></g></svg>',
        "frankenmermaid",
      );
  ...
  record(
    "native_dnf_state_and_er_are_source_verifiable",
    (() => {
      const nodes = (ids) =>
        ids
          .map(
            (id, index) =>
              `<g id="fm-node-${id}-${index}" class="fm-node" data-id="${id}"><rect x="${index * 20}" y="0" width="10" height="10"/></g>`,
          )
          .join("");
  ...
  record(
    "sequence_source_requires_actor_identity_and_message_multiplicity",
    (() => {
      const source =
        "sequenceDiagram\n  participant Alice\n  participant Bob\n  Alice->>Bob: ping\n  Bob-->>Alice: ping\n";
      const completeSvg =
      ...
      return (
        ...
        dropped.checks.some(
          (check) =>
            check.invariant === "sequence_message_count_vs_input__frankenmermaid" &&
            check.pass === false,
        ) &&
        dropped.checks.some(
          (check) =>
            check.invariant === "sequence_source_tokens_vs_output__frankenmermaid" &&
            check.pass === false,
        )
      );
    })(),
  ...
  record(
    "unsupported_source_is_not_decodable",
    groundTruth("journey\n  title Untested\n") === null,
    null,
  );
...
if (
  process.argv[1] &&
  process.argv[1].endsWith("svg_equivalence.mjs") &&
