...
if (!baseBin || !candBin || !nullBin || !corpus || !dumpRoot) {
  console.error(
    "usage: --base <elf> --cand <elf> --null <elf> --corpus <json> --dump-root <dir>\n" +
...
if (disk.base === disk.cand) {
  console.error("REFUSED: base and cand are the same ELF");
  process.exit(3);
}
...
for (const arm of ["base", "cand"]) {
  ...
  if (r.selfElf !== disk[arm]) {
    console.error(
      `REFUSED: ${arm} self-reported ${r.selfElf} but the file on disk is ${disk[arm]}`,
    );
    process.exit(3);
  }
...
const revFiles = (dir) =>
  fs
    .readdirSync(dir)
    .filter((f) => /\.rev\d+\.default\.svg$/.test(f))
    .sort();
...
if (baseRevs.length === 0 || baseRevs.join() !== candRevs.join()) {
  console.error(
    `REFUSED: the two arms dumped different revision sets (${baseRevs.length} vs ${candRevs.length})`,
  );
  process.exit(3);
}
const moved = [];
...
if (expectMoved.length) {
  ...
  if (!same) {
    console.error(
      `REFUSED: expected exactly [${expectMoved.join(",")}] to move, got [${moved.join(",")}]`,
    );
    process.exit(4);
  }
...
for (let round = 0; round < rounds; round += 1) {
  ...
  for (const arm of order) {
    ...
    if (r.instructions < 1e8) {
      console.error(`REFUSED: ${arm} retired only ${r.instructions} instructions`);
      process.exit(3);
    }
    if (r.items === 0) {
    ...
    else if (joints[arm] !== r.joint) {
      console.error(`REFUSED: ${arm} is non-deterministic across rounds`);
      process.exit(4);
    }
  }
  if (joints.base !== joints.null) {
    console.error("REFUSED: the null arm did not reproduce the baseline output");
    process.exit(4);
  }
  console.log(
    `round ${round} order=${order.join(",")} base=${seen.base.instructions} cand=${seen.cand.instructions} null=${seen.null.instructions}  A/B=${(seen.cand.instructions / seen.base.instructions).toFixed(6)}  A/A=${(seen.null.instructions / seen.base.instructions).toFixed(6)}`,
  );
}
...
const summarize = (label, xs) => {
  const s = [...xs].sort((a, b) => a - b);
  console.log(
    `${label}: median ${median(xs).toFixed(6)}  range ${s[0].toFixed(6)}-${s[s.length - 1].toFixed(6)}  n=${xs.length}`,
  );
};
console.log("");
