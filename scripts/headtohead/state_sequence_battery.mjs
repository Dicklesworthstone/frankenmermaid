...
function summarizeState(db) {
  ...
  const rows = [];
  const entries =
    states instanceof w.Map || states instanceof Map
      ? [...states.entries()]
      : Object.entries(states);
  for (const [id, state] of entries) {
    const note = state.note
      ? ` note[${state.note.position}]="${[].concat(state.note.text ?? []).join("|")}"`
      : "";
...
function summarizeSequence(db) {
  ...
  const entries =
    actors instanceof w.Map || actors instanceof Map
      ? [...actors.entries()]
      : Object.entries(actors);
  const actorRows = entries.map(
    ([id, actor]) =>
      `${id}{desc="${actor.description ?? ""}" type=${actor.type ?? ""}${actor.links && Object.keys(actor.links).length ? " links" : ""}}`,
  );
  const messages = (db.getMessages() ?? []).map(
    (message) =>
      `${message.from ?? ""}->${message.to ?? ""}:t${message.type}${message.message ? `:"${message.message}"` : ""}`,
  );
  const boxes = (db.getBoxes?.() ?? []).map(
    (box) => `${box.name ?? ""}[${(box.actorKeys ?? []).join(",")}]`,
  );
...
if (which === "all" || which === "sequence")
  families.push(["sequence", SEQUENCE, summarizeSequence]);
