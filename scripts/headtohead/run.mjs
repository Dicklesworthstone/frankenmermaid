...
const PARSE_CALIBRATION_TARGET_NS = PARSE_MIN_SAMPLE_NS + Math.ceil(PARSE_MIN_SAMPLE_NS / 2);
...
const HOST_WIDE_MAX_BUSY_FRACTION = 0.2;
...
function observedClockComparability(phases, limit) {
  const mean = (prefix) => {
    const values = phases
      .filter((phase) =>
        prefix === "mermaid"
          ? phase.phase.startsWith("mermaid")
          : !phase.phase.startsWith("mermaid"),
      )
...
function classifyHostWideQuiescence(busyRecords, allowedCpus, limit) {
  ...
  return {
    ...
    observed_max_busy_fraction:
      measuredRaw.length > 0
...
function validExclusiveHostClaim(value) {
  return typeof value === "string" && /^trj-booking:[1-9]\d*$/.test(value);
}
...
function sortedUniqueNumbers(values) {
  return [...new Set(values.filter((value) => Number.isSafeInteger(value)))].sort(
...
function hostTopology(cpuRecords) {
  ...
  return {
    ...
    online_cpus:
      online.length > 0 ? online : Array.from({ length: logicalThreads }, (_, cpu) => cpu),
    ...
    affinity_source:
      affinityList.length > 0 ? "linux_proc_status" : `${platform()}_visible_cpu_fallback`,
  };
}
...
function linuxCpuPowerPolicy() {
  ...
  const policies = policyNames.map((policy) => {
    ...
    const affectedRaw =
      readText(join(root, "affected_cpus")) ?? readText(join(root, "related_cpus"));
    return {
      policy,
  ...
  const coveredCpus = [...new Set(policies.flatMap((policy) => policy.affected_cpus))].sort(
    (left, right) => left - right,
  );
  ...
  const coverageComplete =
    onlineCpus.length > 0 && onlineCpus.every((cpu) => coveredCpus.includes(cpu));
  const consistent =
    policies.length > 0 &&
    drivers.length === 1 &&
    governors.length === 1 &&
...
function validCpuPowerPolicy(record) {
  ...
  if (
    !record.boost ||
    !(typeof record.boost.enabled === "boolean" || record.boost.enabled === null) ||
    !(typeof record.boost.source === "string" || record.boost.source === null)
  ) {
    return false;
  }
  if (
  ...
  const coveredCpus = [...new Set(record.policies.flatMap((policy) => policy.affected_cpus))].sort(
    (left, right) => left - right,
  );
  const coverageComplete =
  ...
  const eppPresenceConsistent =
    energyPerformancePreferences.length === 0 ||
    record.policies.every((policy) => typeof policy.energy_performance_preference === "string");
  const consistent =
    drivers.length === 1 &&
    governors.length === 1 &&
    energyPerformancePreferences.length <= 1 &&
...
function sameCpuPowerPolicy(left, right) {
  return (
    ...
    JSON.stringify(powerPolicyComparable(left)) === JSON.stringify(powerPolicyComparable(right))
  );
}
...
function fingerprint() {
  ...
  return {
    ...
    git_untracked_count: allStatus.split("\n").filter((line) => line.startsWith("??")).length,
    rustc: sh("rustc", ["--version"]),
    cargo_profile:
      "release (opt-level=3 for fm-core/parser/layout/render-svg, lto=fat, codegen-units=1)",
    rustflags:
      process.arch === "x64"
    ...
    load_warning:
      loadavg()[0] > cpu.length / 4
        ? "elevated load; absolute timings are not comparable across runs"
        : null,
  };
}
...
function bootstrapMedianRatioCi(numeratorSamples, denominatorSamples) {
  ...
  const denominator = Array.isArray(denominatorSamples)
    ...
    : [];
  const sufficient =
    numerator.length >= MIN_EFFECT_SAMPLES && denominator.length >= MIN_EFFECT_SAMPLES;
  const estimate =
  ...
  for (let iteration = 0; iteration < EFFECT_BOOTSTRAP_RESAMPLES; iteration++) {
    ...
    ratios[iteration] = sampleMedian(numeratorResample) / sampleMedian(denominatorResample);
  }
  ratios.sort((a, b) => a - b);
...
function validBuildRevision(record) {
  return (
    typeof record?.build_git_revision === "string" &&
    /^[0-9a-f]{40}$/.test(record.build_git_revision)
  );
}
...
function balancedSquarePhaseIndexes(schedule = BALANCED_SQUARE) {
  ...
  return {
    fm_before: schedule
      .map((arm, index) => (arm === "fm" && index < 4 ? index : null))
...
function validRustThreadProvenance(record, requestedThreads) {
  ...
  return (
    ...
    record.thread_probe?.method === "instrumented_caller_worker_union_over_one_exact_workload" &&
...
function normalizeParseTypes(values) {
  ...
  return {
    ...
    normalized:
      unknown.length === 0 ? values.map((value) => PARSE_TYPE_NORMALIZATION.get(value)) : null,
...
function medianCiGate(claimRatio, controls, effectCi = null, effectCiRequired = false) {
  ...
  const effectCiAvailable =
    effectCi?.sufficient !== false &&
  ...
  const clause1 =
    effectCiExcludesOne === true || (!effectCiRequired && effectCiExcludesOne === null);
...
function fmBracket(before, after) {
  ...
  return {
    ...
    cv_gate: BRACKET_MAX_CV_PCT,
    reason:
      driftMagnitude <= maxDecidable ? null : "Rust phase drift clears its A/A median-CI floor",
...
if (has("self-test")) {
  ...
  // host: a rule that can only run on the machine it guards is a rule nobody checks.
  {
    ...
    if (
      clockComparabilityVerdict(1.5, limit) ===
      clockComparabilityVerdict(Number((1 / 1.5).toFixed(3)), limit)
    ) {
  ...
  if (
    JSON.stringify(squarePhases) !==
    JSON.stringify({ fm_before: [0, 3], mjs: [1, 2, 5, 6], fm_after: [4, 7] })
  ) {
    throw new Error("balanced-square phase partition regression");
  }
  if (
    JSON.stringify(runBalancedSquare((arm, slot) => `${arm}${slot}`)) !==
    JSON.stringify([
      { arm: "fm", slot: 0, value: "fm0" },
      { arm: "mjs", slot: 1, value: "mjs1" },
      { arm: "mjs", slot: 2, value: "mjs2" },
      { arm: "fm", slot: 3, value: "fm3" },
      { arm: "fm", slot: 4, value: "fm4" },
      { arm: "mjs", slot: 5, value: "mjs5" },
      { arm: "mjs", slot: 6, value: "mjs6" },
      { arm: "fm", slot: 7, value: "fm7" },
    ])
  )
    throw new Error("balanced-square runner regression");
  if (
    JSON.stringify(balancedSquareArmValues(runBalancedSquare((arm, slot) => slot))) !==
    JSON.stringify({
      fm: [0, 3, 4, 7],
      mjs: [1, 2, 5, 6],
    })
  ...
  for (const [ratio, controls, want] of cases) {
    ...
    if (got !== want)
      throw new Error(`median-CI gate regression: ratio=${ratio} want=${want} got=${got}`);
  }
  ...
  if (
    ...
    throw new Error(
      `required effect-CI sample floor regression: ${JSON.stringify(insufficientEffect)}`,
    );
  }
  ...
  if (
    !validElfSelfReport(validElf) ||
    validElfSelfReport({ ...validElf, elf_sha256: "unavailable" })
  ) {
  ...
  if (!has("self-test") && (!validCpuPowerPolicy(livePowerPolicy) || !validCpuIsa(liveIsa))) {
    throw new Error(
  ...
  if (
    ...
    throw new Error(
      `live scheduler-time provenance is invalid: ${JSON.stringify(liveSchedulerTotals)}`,
    );
  }
  ...
  const quietHost = classifyHostWideQuiescence(busySample, [0], HOST_WIDE_MAX_BUSY_FRACTION);
  const busyHost = classifyHostWideQuiescence(busySample, [0, 1], HOST_WIDE_MAX_BUSY_FRACTION);
  ...
  if (
    ...
    validIncumbentThreadProvenance({
      ...
      thread_count_actually_used: null,
    }) ||
  ...
  if (
    ...
    validTimedRenderWorkPolicy({
      ...validTimedRender,
      timed_render_reuses_output_snapshot: true,
    }) ||
  ...
  if (
    ...
    semanticWorkGate(semanticRecord, { ...semanticRecord, revisions: 1_999 }).verdict !==
      "mismatch" ||
    semanticWorkGate(semanticRecord, { ...semanticRecord, input_sha256: "b".repeat(64) })
      .verdict !== "mismatch"
  ) {
  ...
  if (measurementMode === "parse") {
    ...
    if (
      ...
      semanticWorkGate({ ...semanticRecord, parse_recovery_revisions: 1 }, semanticRecord)
        .verdict !== "mismatch" ||
      semanticWorkGate(semanticRecord, { ...semanticRecord, parse_nonempty_config_revisions: 1 })
        .verdict !== "mismatch" ||
      semanticWorkGate(semanticRecord, { ...semanticRecord, parse_deterministic_output: false })
        .verdict !== "mismatch"
    ) {
    ...
    const floorRecord = {
      ...
      null_control: { n: 2 },
      effect_integrated_samples_ns: [PARSE_MIN_SAMPLE_NS, PARSE_MIN_SAMPLE_NS + 1],
      null_integrated_samples_ns: Array.from({ length: 4 }, () => PARSE_MIN_SAMPLE_NS),
      effect_batches: [4, 4],
  ...
  if (
    nativeDnfValidationStatus({
      status: "incumbent_did_not_render",
      native_output_validation: { verdict: "pass" },
    }) !== "native_verified" ||
    nativeDnfValidationStatus({ status: "incumbent_did_not_render" }) !==
      "missing_native_validation" ||
    nativeDnfValidationStatus({
      status: "compared",
      native_output_validation: { verdict: "pass" },
    }) !== "not_dnf_artifact"
  ...
  if (
    preflightOutputProof({ status: "verified" }, { status: "missing_native_validation" }).status !==
      "verified" ||
    preflightOutputProof({ status: "no_matching_artifact" }, { status: "native_verified" })
      .status !== "native_dnf_verified" ||
    preflightOutputProof({ status: "divergent" }, { status: "native_invalid" }).status !==
    ...
    while (stack.length > 0) {
      ...
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        try {
          ...
          /* a malformed or half-written record is not a held slot */
        }
      }
    }
  }
...
if (fleetSlots === null) {
  ...
  if (others.length > 0 && !has("allow-unslotted")) {
    for (const slot of others.slice(0, 5)) {
      console.error(
        `[run] ${slot.agent} holds ${slot.project}/${slot.slot} until ${slot.expires_ts}`,
      );
    }
    console.error(
      "[run] REFUSING: measurements are ONE AT A TIME FLEET-WIDE. Wait for release, or",
    );
...
for (const [id, v] of corpus) {
  ...
  else if (pinned[id] !== v.sha256)
    drift.push(`${id}: pinned ${pinned[id].slice(0, 12)} != generated ${v.sha256.slice(0, 12)}`);
}
if (drift.length > 0) {
  console.error("[run] corpus drift detected -- the baseline would move silently:");
  for (const d of drift) console.error(`       ${d}`);
  console.error(
    "[run] if the change is intentional, re-pin with: node scripts/headtohead/run.mjs --update-pins",
  );
  process.exit(3);
}
...
function positiveIntList(raw) {
  ...
  if (
    ...
    console.error(
      `[run] --thread-sweep must be a unique comma-separated list of positive integers, got ${raw}`,
    );
    process.exit(2);
  }
  return values;
...
const equivalenceDir = arg(
  "equivalence-dir",
  join(REPO, ".benchmarks", "headtohead", "equivalence"),
);
if (measurementMode === "parse" && allowUnverifiedOutput) {
  console.error(
    "[run] --mode parse rejects --allow-unverified-output; linked semantic equivalence is mandatory",
  );
  process.exit(2);
}
...
if (threadSweep.length > 0) {
  ...
  if (typeof fmBuilder !== "string" || fmBuilder.trim().length === 0) {
    console.error(
      "[run] --thread-sweep requires --fm-builder <rch-worker-id> for executable provenance",
    );
    process.exit(2);
  }
}
...
env.measurement_boundary =
  measurementMode === "parse" ? "public_parse_validate" : "parse_layout_render_svg";
console.error(
  `[run] rev=${env.git_rev?.slice(0, 8)}${env.git_dirty ? "-dirty" : ""} load1=${env.loadavg_1m.toFixed(2)} cpus=${env.cpu_count}`,
);
...
const powerAndIsaValid = validCpuPowerPolicy(env.power_policy) && validCpuIsa(env.isa);
if (powerAndIsaRequired && !powerAndIsaValid) {
  console.error(
...
if ((threadSweep.length > 0 || measurementMode === "parse") && !validHostTopology(env)) {
  console.error(
    "[run] INVALID: measured parse rows and thread sweeps require host identity, physical/logical topology, RAM, NUMA, and affinity provenance",
  );
  process.exit(2);
}
...
if (threadSweep.length > 0 && pinArg !== "off") {
  console.error(
    "[run] --thread-sweep requires --pin-cpu off; a one-CPU affinity mask invalidates scaling evidence",
  );
  process.exit(2);
}
let pin = null;
...
function requireHostWideQuiescence(label) {
  for (let attempt = 1; attempt <= HOST_WIDE_QUIET_MAX_ATTEMPTS; attempt++) {
    ...
    const powerPolicyMatchesBaseline =
      livePowerPolicyValid && sameCpuPowerPolicy(env.power_policy, livePowerPolicy);
    ...
    const record = {
      ...
      verdict:
        cpuClassification.verdict === "clear" && powerPolicyMatchesBaseline ? "clear" : "blocked",
    };
...
const [fmCmd, fmArgs] = pin
  ? ["taskset", ["-c", String(pin.cpu), fmBin, corpusPath]]
  : [fmBin, [corpusPath]];
...
if (
  fmProvenance.code !== 0 ||
  provenanceBinaryRecords.length !== 1 ||
  !validElfSelfReport(provenanceBinary)
) {
  console.error(
    "[run] INVALID: untimed frankenmermaid provenance probe did not self-report one valid ELF",
  );
  process.exit(2);
}
...
if (
  fmBuildBase === null ||
  provenanceBinary.build_git_revision !== fmBuildBase ||
  fmBuildBase !== env.git_rev
) {
  console.error(
    "[run] INVALID: benchmark ELF build revision must match both --fm-build-base and checked-out HEAD",
  );
  process.exit(2);
}
...
const preflightOutputProofs = items.map((item) => {
  ...
  const equivalence = findEquivalenceVerdict(
    item.id,
    inputSha,
...
const missingPreflightProofs = preflightOutputProofs.filter(
  (proof) => proof.status === "missing_output_proof",
);
...
console.error(
  `[run] fm elf sha256=${String(env.fm_elf_sha256).slice(0, 16)} (${env.fm_elf_bytes} bytes)`,
);
const expectedBinaryReports = threadSweep.length > 0 ? threadSweep.length : 1;
const beforeReportedThreads = new Set(binaryRecordsBefore.map((record) => record.worker_threads));
const elfSelfReportBeforeValid =
  ...
  binaryRecordsBefore.every(
    (record) =>
      ...
      (threadSweep.length === 0 || record.min_sample_ns === THREAD_SWEEP_MIN_SAMPLE_NS) &&
      (threadSweep.length === 0 ||
...
if (!elfSelfReportBeforeValid) {
  console.error(
    "[run] INVALID: every frankenmermaid sweep arm must self-report the same executing ELF",
  );
}
...
const mjs = has("skip-mermaid")
  ? { records: [], code: 0 }
  : timedPhase("mermaid-js", () =>
      incumbentCpuSet && incumbentCpuSet.cpus.length > 0
        ? runJsonl("mermaid-js", "taskset", [
            "-c",
            incumbentCpuSet.cpus.join(","),
            process.execPath,
            ...mjsArgs,
          ])
...
const afterReportedThreads = new Set(binaryRecordsAfter.map((record) => record.worker_threads));
...
function findEquivalenceVerdict(id, inputSha, elfSha, bundleSha) {
  let names;
  try {
    names = readdirSync(equivalenceDir).filter(
      (n) => n.startsWith("equivalence-") && n.endsWith(".json"),
    );
  } catch {
  ...
  return {
    ...
    directory: equivalenceDir,
    rejected: candidates.slice(0, 4),
  };
}
...
const fmKey = (id, threads) => (threadSweep.length > 0 ? `${id}@t${threads}` : id);
const byFmKey = (recs) =>
  new Map(
    recs
  ...
  const phases =
    threadSweep.length === 0
      ...
      : has("skip-mermaid")
        ...
        : [`frankenmermaid-before-t${threads}`, "mermaid-js", `frankenmermaid-after-t${threads}`];
...
function phaseSweepIdentity(records) {
  ...
  for (const item of items) {
    const scalar = records.find(
      (record) => record.id === item.id && record.worker_threads === 1 && record.status === "ok",
    );
...
if (sweepOutputIdentity) {
  sweepOutputIdentity.verdict =
    sweepOutputIdentity.before.verdict === "pass" && sweepOutputIdentity.after.verdict === "pass"
      ? "pass"
      : "fail";
}
...
for (const { item, threads } of measurements) {
  ...
  const row = {
    ...
    fm_build: buildProvenanceRecord(fmBuildMethod, fmBuilder, fmBuildBase, fmBuildCleanOverlay),
  ...
  if (
    ...
    rows.push({
      ...
      error: "parse rows require an observed single calling thread over the exact parse workload",
    });
    continue;
  }
  ...
  if (
    ...
    (measurementMode === "render" && fBefore.output_sha256_lean !== fAfter.output_sha256_lean)
  ) {
    hardFail = true;
    rows.push({
      ...row,
  ...
  row.lean_slowdown =
    measurementMode === "render" ? f.pipeline_lean_ns.p50 / f.pipeline_ns.p50 : null;
  ...
  if (measurementMode === "parse" && !validParseSampleFloor(m, true)) {
    hardFail = true;
    rows.push({
      ...row,
      status: "sample_floor_violation",
  ...
  row.effect_ci_required = measurementMode === "parse" || item.effect_ci_required === true;
  row.effect_ci = bootstrapMedianRatioCi(mTiming.samples, fTiming.samples);
  ...
  row.speedup_lean = measurementMode === "render" ? mTiming.p50 / f.pipeline_lean_ns.p50 : null;
  row.bytes_ratio = measurementMode === "render" ? m.output_bytes / f.output_bytes : null;
  row.bytes_ratio_lean = measurementMode === "render" ? m.output_bytes / f.output_bytes_lean : null;
...
const elfShaForEquivalence = /^[0-9a-f]{64}$/.test(String(env.fm_elf_sha256))
  ? env.fm_elf_sha256
  : null;
for (const row of ok) {
...
const equivalenceFailures = ok
  ...
  .map(
    (r) =>
      `${threadSweep.length > 0 ? `${r.id}@t${r.fm_worker_threads}` : r.id}:${r.output_equivalence.status}`,
  );
...
const dnfValidationFailures = dnf
  ...
  .map(
    (row) =>
      `${threadSweep.length > 0 ? `${row.id}@t${row.fm_worker_threads}` : row.id}:${row.native_output_validation.status}`,
  );
...
const speedupMinAggregate = speedupsMin.length
  ...
  : null;
const rowLabel = (row) => (threadSweep.length > 0 ? `${row.id}@t${row.fm_worker_threads}` : row.id);
...
const hostWideExclusivity = {
  verdict:
    ...
    finalHostWideChecks.every((check) => check?.verdict === "clear")
      ...
      : "blocked",
  ...exclusiveHostClaimProvenance(exclusiveHostClaim),
  claim_reference_format:
    exclusiveHostClaim === null ? null : "trj-booking:<Agent-Mail-CLAIM-message-id>",
  complete_host_cpuset: env.affinity_cpus.length === env.logical_threads,
  maximum_busy_fraction: HOST_WIDE_MAX_BUSY_FRACTION,
  sample_ms: HOST_WIDE_QUIET_SAMPLE_MS,
  maximum_admission_attempts: HOST_WIDE_QUIET_MAX_ATTEMPTS,
...
const summary = {
  ...
  environment_provenance_gate: {
    ...
    rule: "cross-engine evidence requires complete ISA flags and one internally consistent full-CPU power policy",
    isa: env.isa,
    power_policy: powerPolicySummary(env.power_policy),
  },
  ...
  dnf: dnf.map((r) => ({
    id: r.id,
    budget_ms: r.mjs_budget_ms,
    phase: r.mjs_dnf_phase,
    fm_p50_ns: r.fm_p50_ns,
    speedup_lower_bound: r.speedup_lower_bound,
    error: r.error,
  ...
  output_equivalence_gate: {
    verdict:
      dnfValidationFailures.length > 0
        ? "fail"
        : equivalenceFailures.length === 0
          ? "pass"
          : allowUnverifiedOutput
            ? "admitted_unverified"
            : "fail",
    rule:
      "every measured ratio row needs a passing cross-engine equivalence verdict from the same " +
    ...
    method:
      (measurementMode === "parse" ? "linked_parse_semantics_via_" : "") +
    ...
    caveat:
      equivalenceFailures.length > 0 && allowUnverifiedOutput && dnfValidationFailures.length === 0
  ...
  median_ci_gate_rule:
    "effect CI excludes 1.0 when required; claim magnitude >= max(1.01, " +
  ...
  fm_bracket_undecidable: ok.filter((r) => r.fm_bracket.verdict === "undecidable").map(rowLabel),
  ...
  fm_bracket_gate_failures: ok.filter((r) => r.fm_bracket.verdict !== "pass").map(rowLabel),
  thread_sweep:
    threadSweep.length > 0
      ? {
          threads: threadSweep,
  ...
  cv_gate: "never",
  median_ci_gate_failures: ok.filter((r) => r.median_ci_gate.verdict === "fail").map(rowLabel),
...
console.log("");
console.log(
  `corpus=${items.length}  mermaid=${PINS.mermaid.version} (securityLevel=${PINS.mermaid.security_level})  rev=${env.git_rev?.slice(0, 8)}`,
);
console.log("");
console.log(
  `${pad("item", 22)}${lpad("nodes", 6)}${lpad("edges", 7)}${lpad("fm p50 ms", 12)}${lpad("mermaid ms", 12)}${lpad("speedup", 10)}${lpad("(by min)", 10)}${lpad("fm mad%", 9)}${lpad("bytes x", 9)}${lpad("lean x", 8)}  gate`,
);
...
for (const r of rows) {
  ...
  if (r.status === "comparator_dnf") {
    ...
    const nativeGate = r.content_verified
      ? "dnf-native"
      : `dnf-${r.native_output_validation.status}`;
    console.log(
  ...
  const leanBytesRatio = Number.isFinite(r.bytes_ratio_lean)
    ? `${r.bytes_ratio_lean.toFixed(2)}x`
    : "-";
  console.log(
    pad(displayId, 22) +
      lpad(r.nodes, 6) +
      lpad(r.edges, 7) +
      lpad(ms(r.fm_p50_ns), 12) +
      lpad(ms(r.mjs_p50_ns), 12) +
      lpad(`${r.speedup.toFixed(0)}x`, 10) +
      lpad(`${r.speedup_min.toFixed(0)}x`, 10) +
      lpad(r.fm_mad_pct.toFixed(1), 9) +
      lpad(bytesRatio, 9) +
      lpad(leanBytesRatio, 8) +
...
if (summary.thread_sweep) {
  console.log(
    "frankenmermaid caller-thread scaling (one persistent pool; scalar hash identity required):",
  );
  for (const r of rows.filter((row) => row.status === "ok" || row.status === "fm_only")) {
...
const displayedMinSpeedup = summary.speedup_min_estimator ?? summary.parse_speedup_min_estimator;
if (displayedSpeedup) {
  console.log(
    `${measurementMode} speedup vs mermaid ${PINS.mermaid.version} (p50):  min ${displayedSpeedup.min.toFixed(0)}x  median ${displayedSpeedup.median.toFixed(0)}x  max ${displayedSpeedup.max.toFixed(0)}x`,
  );
  console.log(
    `${measurementMode} speedup vs mermaid ${PINS.mermaid.version} (min):  min ${displayedMinSpeedup.min.toFixed(0)}x  median ${displayedMinSpeedup.median.toFixed(0)}x  max ${displayedMinSpeedup.max.toFixed(0)}x`,
  );
}
...
for (const r of ok.filter((x) => x.class !== "doc_build" && x.revisions > 1)) {
  ...
  console.log(
    `${r.class} ${r.id}: ${r.revisions} revisions -- per ${unit} frankenmermaid ` +
      ...
      (measurementMode === "parse"
        ? "(equal-work public parser boundary.)"
...
if (dnf.length) {
  ...
  for (const r of dnf) {
    console.log(
      `  ${pad(r.id, 22)} ${r.nodes} nodes / ${r.edges} edges -- ${r.mjs_dnf_phase} phase, budget ${(r.mjs_budget_ms / 1000).toFixed(0)}s`,
    );
    if (r.mjs_dnf_kind === "timeout") {
      ...
      if (Number.isFinite(r.speedup_lower_bound)) {
        console.log(
          `  ${" ".repeat(22)} frankenmermaid: ${ms(r.fm_p50_ns)} ms, so the speedup is at least ${r.speedup_lower_bound.toFixed(0)}x -- a bound, not a measurement.`,
        );
      } else {
        console.log(
      ...
      console.log(
        `  ${" ".repeat(22)} FAILED${failureClass} after ${(r.mjs_elapsed_ms / 1000).toFixed(1)}s: ${r.error}`,
      );
      console.log(
...
if (leanSlow.length) {
  ...
  console.log(
    `note: the lean output profile is smaller but SLOWER on ${leanSlow.length}/${ok.length} items (worst ${worst.id}: ${worst.lean_slowdown.toFixed(2)}x) -- A11yConfig::none() falls off the streaming fast path.`,
  );
}
...
if (equivGate.verdict !== "pass") {
  console.log("");
  console.log(
    `OUTPUT EQUIVALENCE GATE ${equivGate.verdict === "fail" ? "FAIL" : "ADMITTED UNVERIFIED"}: ` +
  ...
  for (const r of ok.filter((row) => !row.content_verified)) {
    ...
    if (e.status === "divergent") {
      console.log(
        `  ${r.id}: ${e.equivalent}/${e.diagrams} diagrams equivalent; ` +
          `divergent/unverified families: ${e.divergent_families.join(", ")}`,
      );
    } else {
      console.log(
        `  ${r.id}: ${e.status}${e.reason ? ` (${e.reason})` : ""} -- run scripts/headtohead/equivalence.mjs`,
      );
    }
  }
...
if (asym) {
  console.log("");
  console.log(
    `PHASE-LOAD PROVENANCE (${asym.rule}): CPU-busy ratio ${asym.busy_ratio?.toFixed(2)}, heavier on ${asym.heavier_phase}.`,
  );
}
console.log(`\nevents:  ${jsonlPath}`);
