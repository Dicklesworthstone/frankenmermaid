...
const PINS = JSON.parse(
  fs.readFileSync("/data/projects/frankenmermaid/scripts/headtohead/pins.json", "utf8"),
);
const ROOT = path.join(os.homedir(), "snap", "chromium", "common");
async function launch() {
  const p = fs.mkdtempSync(path.join(ROOT, "fm-st-"));
  const proc = spawn(
    PINS.chromium.binary,
    [
      "--headless=new",
  ...
  proc.stderr.on("data", (c) => {
    e += String(c);
    const m = e.match(/DevTools listening on ws:\/\/127\.0\.0\.1:(\d+)/);
...
await send("Runtime.evaluate", {
  expression: fs.readFileSync("/home/ubuntu/.cache/fm-headtohead/mermaid-11.15.0.min.js", "utf8"),
});
let i = 0;
