...
function selfTest() {
  ...
  const record = (name, ok, detail) => {
    cases.push(name);
    if (!ok)
      throw new Error(`golden svg health self-test failed: ${name} (${JSON.stringify(detail)})`);
  };
  ...
  record(
    "a_healthy_document_has_no_violations",
  ...
  record(
    "dangling_marker_reference_is_a_violation",
    has(wrap('<defs></defs><path marker-end="url(#arrowhead)"/>'), "dangling_reference"),
    "bd-8zo0 shape",
  );
  record(
    "dangling_href_reference_is_a_violation",
    has(wrap('<use href="#missing"/>'), "dangling_reference"),
    "href form",
  );
  record(
    "duplicate_id_is_a_violation",
    has(wrap('<rect id="a"/><rect id="a"/>'), "duplicate_id"),
    "two rects share an id",
  );
  record(
    "nan_coordinate_is_a_violation",
    has(wrap('<rect x="NaN" y="1"/>'), "non_finite_number"),
    "NaN x",
  );
  record(
    "infinite_dimension_is_a_violation",
    has(wrap('<rect width="Infinity"/>'), "non_finite_number"),
    "Infinity width",
  );
  record(
    "bare_ampersand_is_a_violation",
    has(wrap("<text>a &amp b</text>"), "bare_ampersand"),
    "unescaped &",
  );
  record(
    "escaped_entities_are_not_violations",
    !has(wrap("<text>a &amp; b &#39; c &#x27; d</text>"), "bare_ampersand"),
    "named, decimal and hex entities",
  );
  record(
    "degenerate_viewbox_is_a_violation",
    svgHealthViolations('<svg viewBox="0 0 0 10"></svg>').some((v) => v.check === "bad_viewbox"),
    "zero width",
  );
  record(
    "absent_viewbox_is_a_violation",
    svgHealthViolations("<svg></svg>").some((v) => v.check === "bad_viewbox"),
    "no viewBox",
  );
...
function scan() {
  ...
  if (scanned === 0) {
    console.error(
      "[golden-health] no golden SVGs found in either corpus; refusing to report a pass",
    );
    process.exit(2);
  }
