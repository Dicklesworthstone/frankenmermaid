...
### Render: avoid per-edge offset point Vec before smooth path serialization — KEPT, medium-wide +9.3% (2026-06-27)
- **Lever:** `render_edge` and the Gantt dependency path used to allocate a fresh
  `Vec<(f32, f32)>` per edge only to add `offset_x`/`offset_y` before calling the
  smooth path writer. Replaced that materialization with `build_smooth_path_by`,
  which reads each `LayoutEdgePath` point by index and applies the offset lazily
  while emitting the same `d` bytes. Added
  `smooth_path_by_index_matches_expected_bytes` to pin the emitted cubic path.
...
- **Measured (per-crate `wide_stages/render`, `rch exec`, worker `hz2`, target dir
  `/data/projects/.rch-targets/frankenmermaid-cod-b`, package
  `frankenmermaid-cli`, bench `pipeline_bench`; both baseline and candidate were
  built with distinct `RUSTFLAGS=-C metadata=...` tags to avoid shared-artifact
  contamination):**
  - ORIG/current main `93152f1`: `8x16 709.78 us`, `12x24 1.7183 ms`,
    `16x32 3.3505 ms`.
  - Candidate: `8x16 706.42 us`, `12x24 1.5589 ms`, `16x32 3.3278 ms`.
  - **Ratio vs ORIG:** `8x16 0.995x` (neutral), `12x24 0.907x` (**9.3% faster**),
    `16x32 0.993x` (neutral). Local fallback runs were noisy and are treated only
    as routing evidence; the remote `hz2` pair is the keep proof.
- **Original comparator:** pinned Mermaid `11.12.0` wide denominators reused from
  ...
  Candidate render-stage ratios are `0.002241x`, `0.001588x`, and `0.001156x`
  (Mermaid.js `446.1x`, `629.8x`, and `865.2x` slower). Baseline ratios were
  `0.002252x`, `0.001750x`, and `0.001164x`.
- **Behavior proof:** `cargo fmt --check -p fm-render-svg` passed; `rch exec --
  cargo clippy --profile release -p fm-render-svg --all-targets -- -D warnings`
  passed on `hz2`; `rch exec -- cargo test --profile release -p fm-render-svg`
  passed on `ovh-a` (`224` tests + doctests); local conformance
  `cargo test --profile release -p frankenmermaid-cli --test
  frankentui_conformance_test` passed (`1` test). Two `rch exec` conformance
  wrappers queued/stalled before launching and were interrupted; the final
  conformance command used the same target dir locally. Output is generated by
  the same smooth-path math and attribute serializers, with the temporary offset
  tuple vector removed.
- **Verdict:** kept. This is a narrow construction win (medium-wide case), neutral
  at small and largest wide sizes, and does not reopen the documented root
