...
use fm_layout::{
    LayoutAlgorithm, LayoutGuardrails, layout_diagram_traced,
    layout_diagram_traced_with_algorithm_and_guardrails,
};
use std::time::Instant;
fn bestn<F: Fn() -> (usize, bool)>(f: F, it: usize) -> (u128, usize, bool) {
    let mut b = u128::MAX;
    let mut n = 0;
    let mut fin = true;
    for _ in 0..it {
...
fn main() {
    ...
    let cases = [
        ("seq", LayoutAlgorithm::Sequence),
        ("sankey", LayoutAlgorithm::Sankey),
        ("pie", LayoutAlgorithm::Pie),
