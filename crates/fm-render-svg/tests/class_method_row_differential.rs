...
//!   * THE CLASSIFIER (bd-r2gll, still open). mermaid does not put `$`/`*` in the text at all — it
//!     returns `text-decoration:underline;` / `font-style:italic;` in `cssStyle` beside it, because
//!     UML underlines a static member and italicises an abstract one. We append the raw character.
//!     Those four rows are carried in the fixture and EXCLUDED from the text comparison here,
//!     because fixing them means adding the styling, not deleting the character — deleting it alone
//!     would lose the static/abstract distinction entirely.
//!
...
fn every_method_row_draws_what_mermaid_draws() {
    ...
    let mut compared = 0;
    for row in fixture() {
        if !row.classifier.is_empty() || DELIBERATE.contains(&row.member.as_str()) {
            continue;
        }
        compared += 1;
