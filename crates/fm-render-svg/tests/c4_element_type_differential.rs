...
///
/// ⚠️ COMPARED CASE-INSENSITIVELY, and that is not a convenience. The impl that actually shipped
/// collapsed `System_Ext` to the CAPITALISED `"System"`, so a check for the lowercase `<<system>>`
/// would have passed against the very code this test exists to reject — a vacuous family member
/// dressed as a control. Folding case makes the assertion answer the question it is named for.
#[test]
fn a_variant_is_not_collapsed_into_its_base_type() {
    for (macro_name, base) in [
        ...
        let collapsed = format!("<<{base}>>");
        let drawn = type_labels(&diagram(macro_name));
        assert!(
            !drawn
                .iter()
                .any(|label| label.eq_ignore_ascii_case(&collapsed)),
            "{macro_name} collapsed to the base type {collapsed:?}: {drawn:?}"
        );
    }
}
