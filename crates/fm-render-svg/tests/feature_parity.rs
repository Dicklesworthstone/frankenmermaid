...
const CASES: &[(&str, &str, &str)] = &[
    // ── class ────────────────────────────────────────────────────────────────────────────────
    (
        "class_interface_annotation",
        "classDiagram\n  class Alpha\n  Alpha : +run()\n",
        "classDiagram\n  class Alpha\n  <<interface>> Alpha\n  Alpha : +run()\n",
    ),
    (
        "class_abstract_annotation",
        "classDiagram\n  class Alpha\n  Alpha : +run()\n",
        "classDiagram\n  class Alpha\n  <<abstract>> Alpha\n  Alpha : +run()\n",
    ),
    (
...
fn no_declared_feature_renders_identically_to_its_absence() {
    ...
    for (name, baseline, variant) in CASES {
        ...
        if identical && !known {
            ignored.push(format!(
                "{name}: the variant renders byte-identically to its baseline"
            ));
        }
        if !identical && known {
