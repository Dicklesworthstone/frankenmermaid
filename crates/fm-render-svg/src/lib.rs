...
pub(crate) fn svg_frame(
    ...
    let title_height = if generic_title_for(ir).is_none() {
        0.0
    } else {
        config.font_size + 22.0
    };
...
/// The title to draw above the diagram, or `None` when this family draws none.
///
/// ⚠️ ONE FUNCTION, TWO CALLERS, AND THEY MUST NOT DRIFT. `svg_frame` reserves a band of height for
/// the title and `render_layout_to_svg` draws into it. When the two disagree the diagram gets a
...
fn render_layout_to_svg(
    ...
    let legend_enabled = is_c4_legend_enabled(ir);
    let legend_height = if legend_enabled { 128.0 } else { 0.0 };
    let generic_title = generic_title_for(ir);
